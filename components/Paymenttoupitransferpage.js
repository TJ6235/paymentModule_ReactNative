...
const Paymenttoupitransferpage = (props) => {
  ...
  const navigation = useNavigation();
    ...
    const saveData = async ()=>{
      ...
      navigation.navigate('Upisuccesspage'); 
      
      try {

        url
        let result = await fetch(`${url}/Upitransactions`, {
          method: "post",
          headers: { 
  ...
  return (
    ...
    <View styles={styles.paymenttoupimaincontainer}>
    <View style={styles.topmaincontainer}>
      ...
      <View style={styles.nameandnumber}>
        <View style={{paddingRight:140}}>
          ...
          <Text style={{marginTop:4,marginLeft:0,fontSize:12}}>{item.Firstuserupiid}</Text>
          </View>
          <Image
...
export default Paymenttoupitransferpage
